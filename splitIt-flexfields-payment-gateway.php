...
/**
 * Check domains to skip notification
 * @return bool
 */
function isExceptionDomainToNotification() {
    $exceptionDomains = array('my-wordpress-blog.local', 'localhost', '127.0.0.1');

    return !in_array(DOMAIN,$exceptionDomains);
}
...
function splitit_activate() {
	...
	if ( isExceptionDomainToNotification() ) {
...
/**
 * Return base curl with default option
 *
 * @param string $url
 ...
 */
function get_base_curl($url,  $post_data  = '', $headers = array())
{
    $curl = curl_init();
...
function send_slack_notification( $message ) {
    $url = 'https://hooks.slack.com/services/' . SLACK_KEY_1 . '/' . SLACK_KEY_2 . '/' . SLACK_KEY_3;
    $post_data = array('payload' => '{
		"blocks": [{
			"type": "section",
			"text": {
				"type": "mrkdwn",
    ...
    $curl = get_base_curl($url, $post_data);
...
function send_slack_refund_notification( $message ) {
    ...
    $curl = get_base_curl($url, $post_data, $headers);
...
require_once 'db/create-async-refund-log-table.php'; //for async refunds
...
function splitit_flexfields_payment_plugin_init_gateway_class() {
	...
	class WC_SplitIt_FlexFields_Payment_Gateway extends WC_Payment_Gateway {
		...
		public function __construct() {
			...
			add_action( 'shutdown', array( $this, 'check_fatal_errors' ) );
		...
		/**
		 * Method that check and print fatal error
		 */
		public function check_fatal_errors() {
			$error = error_get_last();
			if ( $error && isset($error['type']) && $error['type'] === E_ERROR && isset($error['message']) ) {
				echo '<div style="width: 100%; margin: 15px auto; text-align: center; font-size: 16px; background-color: bisque; padding: 10px; border-radius: 10px;">';
				if ( strpos( $error['message'], 'GuzzleHttp') !== false ) {
					preg_match('/\/wp-content\/plugins\/([^\/]+)/', $error['message'], $matches);

					if ( isset($matches[1]) && $matches[1] != 'Splitit.Plugins.WooCommerce.FF' && $matches[1] != 'splitit-installment-payments' ) {
		...
		public function payment_fields() {
            ...
            $sandbox = !('production' === $this->splitit_environment);
			...
			$tmp    = str_replace( '<order_id>', $wp->query_vars['order-pay'] ?? null, $flex_fields_form );
			$tmp3   = str_replace( '<3ds>', (bool) $this->splitit_settings_3d, str_replace( '<debug>', $sandbox, $tmp ) );
		...
		public function handle_create_refund($refund, $order) {
			global $wpdb;

			SplitIt_FlexFields_Payment_Plugin_Log::log_to_file('$order: ' . json_encode($order));
			...
			foreach ($items as $item_id => $item) {
				...
				SplitIt_FlexFields_Payment_Plugin_Log::log_to_file('$item->get_name(): ' . $item->get_name());
				SplitIt_FlexFields_Payment_Plugin_Log::log_to_file('$item->get_meta_data(): ' . json_encode($item->get_meta_data()));
				SplitIt_FlexFields_Payment_Plugin_Log::log_to_file('$item->get_meta(_refunded_item_id): ' . json_encode($item->get_meta('_refunded_item_id')));
			...
			$option_name = 'splitit_refund_data_' . $order_id;
			update_option($option_name, json_encode($refund_data));
		}
			...
			try {
				...
				if ( 'splitit' === $order->get_payment_method() ) {
					...
					if ( in_array( $order->get_status(), array( 'processing', 'completed' ) ) ) {
						...
						return new WP_Error( 'error', "Order with this status cannot be refunded, only cancelled." );
					}
				}
				...
				try {
					...
					if ($order->has_status( 'refunded' )) {
						SplitIt_FlexFields_Payment_Plugin_Settings::update_order_status_to_old( wc_get_order( $order_id ) );
					}
				} catch ( Exception $e ) {
					SplitIt_FlexFields_Payment_Plugin_Log::save_log_info( $data, $e->getMessage(), 'error' );
					return new WP_Error( 'error', "Refund unable to be processed online, consult your Splitit Account to process manually." );
				}
		...
		public function woocommerce_payment_change_order_status( $order_id ) {
			...
			if ( $order->get_payment_method() == 'splitit' ) {
                $order->update_status( ! $this->settings['splitit_auto_capture'] ? 'pending' : 'processing' );
			}
		}
		...
		public function get_access_token( $auth_code ) {
			$environment = get_option( 'splitit_environment' ) ?? $this->splitit_environment;

			$url     = 'https://id.' . $environment . '.splitit.com/connect/token';
			...
			if ( false === $response ) {
				...
				echo '<div style="width: 500px; margin: 0 auto; text-align: center; font-size: 16px; background-color: bisque; padding: 10px; border-radius: 10px;">';
				echo 'Error:</br>';
				echo $auth_code.'</br>';
				echo $response;
				echo '</div>';
		 ...
		 */
		public function get_user_data( $url, $token ) {
            $headers = array(
                'Content-Type: application/json',
                'Authorization: Bearer ' . $token,
            ...
            $curl = get_base_curl($url, '', $headers);
		 ...
		 * @return bool|mixed|string
		 */
		public function get_merchant_settings( $url, $token, $merchant_id ) {
			$header = array(
				'Content-Type: application/json',
				'Authorization: Bearer ' . $token,
		 ...
		 * @return bool|mixed|string
		 */
		public function get_list( $url, $token, $merchant_id = null, $old_api = false ) {
			$header = array(
				'Content-Type: application/json',
				'Authorization: Bearer ' . $token,
				'touch-point: WooCommercePlugin',
			...
			if ( $merchant_id ) {
                $merchant_param = $old_api ? 'merchant-id: ' : 'merchantId: ';
				$header[] = $merchant_param . $merchant_id;
			}
			...
			if ( false === $response ) {
				...
				echo curl_error( $curl );
				echo 'Failed';
				SplitIt_FlexFields_Payment_Plugin_Log::log_to_file( 'get_list Error: url: ' . $url . ', error: ' . $response );
				wp_die();
			} elseif ( property_exists( json_decode( $response ), 'error' ) || property_exists( json_decode( $response ), 'Errors' ) ) {
				echo 'Error:<br />';
				echo $response;
				SplitIt_FlexFields_Payment_Plugin_Log::log_to_file( 'get_list Error: url: ' . $url . ', error: ' . $response );
				wp_die();
		 ...
		 * @return mixed
		 */
		public function get_client_secret( $url, $token, $client_id, $merchant_id, $old_api = false ) {
			$header = array(
				'Accept: text/plain',
            ...
            $curl = get_base_curl($url, $post_data, $header);
			...
			if ( false === $response ) {
				...
				echo curl_error( $curl );
				echo 'Failed';
				SplitIt_FlexFields_Payment_Plugin_Log::log_to_file( 'get_client_secret Error: url: ' . $url . ', error: ' . $response );
				wp_die();
			} elseif ( json_decode( $response )->error || property_exists( json_decode( $response ), 'Errors' ) ) {
				echo 'Error:<br />';
				echo $response;
				SplitIt_FlexFields_Payment_Plugin_Log::log_to_file( 'get_client_secret Error: url: ' . $url . ', error: ' . $response );
				wp_die();
		...
		public function generate_merchants_list_dropdown( $list, $user_data ) {
			$merchants_list = json_decode( $list, true )['MerchantList'];
			...
			<script>
				$(function () {
                    ...
                    const id_token = '<?php echo $_SESSION['id_token'] ?? ''; ?>'
		...
		public function get_pop_up_styles() {
			echo '
				...
				.Path-2 {
					width: 133px;
					height: 14px;
					border-bottom: dashed 2px #20a5a6;
				}
		...
		public function generate_terminals_list_dropdown( $terminals_list, $credentials_list, $merchant_id, $user_data, $env ) {
								...
								<form id="terminalForm" method=POST action="/terminal">
									...
									<select name="terminal_id" id="terminals_list_dropdown" class="merchant-select">
										...
										foreach ( $terminals_list as $item ) {
											...
											echo '<option value="' . $item['TerminalId'] . '">' . $name . '</option>';
										}
										?>
									</select>
			...
			<script>
				$(function () {
					...
					$('#merchants_list_dropdown').change(function () {
						$('#show_terminal').prop('disabled', true);
						$('#merchantForm').submit()
					})
					$('#terminals_list_dropdown').change(function () {
                        ...
                        if (oldTerminalId === selectedTerminalId) {
                            $('#credentials_list_block').hide();
                            $('#credentials_list_dropdown').val('').prop('selected', true);
                            $('#show_terminal').prop('disabled', false);
                            $('#anotherTerminal').hide();
                        } else {
                            $('#show_terminal').prop('disabled', true);

                            if (oldMerchantId && oldMerchantId === selectedMerchantId) {
                                $('#anotherTerminal').show();
                            }

                            $('#credentials_list_block').show().css('margin-top', '20px');
		...
		public function splitit_custom_url_handler() {
            if ( isset( $_SERVER['REQUEST_URI'] ) ) {
                $env = get_option( 'splitit_environment' ) ?? $this->splitit_environment;

                if ( strstr( $_SERVER['REQUEST_URI'], '?', true ) === '/splitit-auth/callback' ) {
                    $data = stripslashes_deep( $_GET );

					if ($data['state'] == 'FORCE_LOGOUT' ) {
                    ...
                    if ( !empty($data['code']) ) {
						...
						if ( $access_token ) {
							...
							$merchant_ref_list = $this->get_list( 'https://pluginproxy.' . $env . '.splitit.com/api/v1/merchant/ref-list?forceRefresh=true&Statuses=Live', $access_token );
				...
				if ( '/get-terminals' === $_SERVER['REQUEST_URI'] ) {
					if ( isset( $_POST ) ) {
						...
						$merchant_settings = $this->get_merchant_settings( 'https://pluginproxy.' . $env . '.splitit.com/api/v1/merchant/extended-info', $access_token, $merchant_id );
						update_option( 'merchant_settings', $merchant_settings );

						if ( $access_token ) {
							$terminals_list = $this->get_list( 'https://pluginproxy.' . $env . '.splitit.com/list', $access_token, $merchant_id );
							$credentials_list = $this->get_list( 'https://pluginproxy.' . $env . '.splitit.com/api/v1/credentials/list', $access_token, $merchant_id );
				...
				if ( '/terminal' === $_SERVER['REQUEST_URI'] ) {
					if ( isset( $_POST ) ) {
						...
						if ( !session_id() ) { session_start(); }
                        $merchants_list = $_SESSION['merchants_list'] ?? null;
                        $terminals_list = $_SESSION['terminals_list'] ?? null;
                        $access_token = $_SESSION['access_token'] ?? null;
							...
							foreach ( $terminals_list as $terminal ) {
								if ( $terminal['TerminalId'] == $terminal_id ) {
									...
									if ( $client_id ) {
										...
										$client_secret = $this->get_client_secret( 'https://pluginproxy.' . $env . '.splitit.com/api/v1/credentials/generate', $access_token, $client_id, $merchant_id );
									...
									update_option( 'splitit_' . $env . '_terminal_id', $terminal['TerminalId'] );
...
<!--					</div>-->
									...
									<div class="page-setting-content-block mt-3">
										...
										<div class="mt-3">
											...
											<input
													...
													value="<?php echo ! is_array( $page_config ) || $page_config['strip']['text_button_color'] == '' ? '#0b8286' : $page_config['strip']['text_button_color']; ?>"
									...
									<div class="page-setting-content-block mt-3">
										...
										<div class="mt-3">
											...
											<input
													...
													value="<?php echo ! is_array( $page_config ) || $page_config['strip']['background_color'] == '' ? '#0b8286' : $page_config['strip']['background_color']; ?>"
									...
									<div class="page-setting-content-block mt-3">
										...
										<div class="mt-3">
											...
											<input
													...
													value="<?php echo ! is_array( $page_config ) || $page_config['banner']['text_button_color'] == '' ? '#0b8286' : $page_config['banner']['text_button_color']; ?>"
													data-name="text_button_color"
											>
										</div>
									...
									<div class="page-setting-content-block mt-3">
										...
										<div class="mt-3">
											...
											<input
													...
													value="<?php echo ! is_array( $page_config ) || $page_config['banner']['banner_bg_color'] == '' ? '#0b8286' : $page_config['banner']['banner_bg_color']; ?>"
										...
										<div class="page-setting-content-block mt-3">
											...
											<div class="mt-3">
												...
												<input
														...
														value="<?php echo ! is_array( $page_config ) || $page_config['logo']['bg_color'] == '' ? '#0b8286' : $page_config['logo']['bg_color']; ?>"
														data-name="bg_color"
												>
											</div>
										</div>
		...
		public function generate_payment_method_settings_section_html( $key, $data ) {
			...
			<div class="main-section payment_method">
				...
				<div id="payment_method_settings_section" class="setting-content-block mt-3 hide">
                    ...
                    <!--start splitit_refund_strategy-->
                    <div class="setting-wrap" style="padding-bottom: 0">
                        <div class="main-section-environment mt-3"
                             <!--style="--><?php /*echo esc_attr( $data['splitit_refund_strategy']['css'] ); */?>"
                        >
                            <div class="setting-title">
								<?php echo wp_kses_post( $data['splitit_refund_strategy']['title'] ); ?>
                            </div>
                            <div>
                                <legend class="screen-reader-text"><span><?php echo wp_kses_post( $data['splitit_refund_strategy']['title'] ); ?></span></legend>

								<?php echo $this->get_description_html( $data['splitit_refund_strategy'] ); // WPCS: XSS ok. ?>

                                <select
                                        class="env-select select <?php echo esc_attr( $data['class'] ); ?>"
                                        name="<?php echo esc_attr( $this->get_field_key( 'splitit_refund_strategy' ) ); ?>"
                                        id="<?php echo esc_attr( $this->get_field_key( 'splitit_refund_strategy' ) ); ?>"
                                        style="<?php echo esc_attr( $data['splitit_refund_strategy']['css'] ); ?>"
									<?php disabled( $data['disabled'], true ); ?>
									<?php echo $this->get_custom_attribute_html( $data['splitit_refund_strategy'] ); // WPCS: XSS ok. ?>
                                >
									<?php foreach ( (array) $data['splitit_refund_strategy']['options'] as $option_key => $option_value ) : ?>
										<?php if ( is_array( $option_value ) ) : ?>
                                            <optgroup label="<?php echo esc_attr( $option_key ); ?>">
												<?php foreach ( $option_value as $option_key_inner => $option_value_inner ) : ?>
                                                    <option
                                                            value="<?php echo esc_attr( $option_key_inner ); ?>"
														<?php selected( (string) $option_key_inner, esc_attr( $this->get_option( 'splitit_refund_strategy' ) ) ); ?>
                                                    >
														<?php echo esc_html( $option_value_inner ); ?>
                                                    </option>
												<?php endforeach; ?>
										...
										<?php else : ?>
                                            <option
                                                    value="<?php echo esc_attr( $option_key ); ?>"
												<?php selected( (string) $option_key, esc_attr( $this->get_option( 'splitit_refund_strategy' ) ) ); ?>
                                            >
												<?php echo esc_html( $option_value ); ?>
                                            </option>
										<?php endif; ?>
		...
		public function generate_merchant_settings_allow_range_title() {
			if ( get_option( 'merchant_settings' ) ) {
				return '<b>Installment range is: <span class="merchant-info">' . get_option( 'merchant_settings' )->MinInstallments . ' to ' . get_option( 'merchant_settings' )->MaxInstallments . '</span> </b><br>
                        <b style="display: block; margin-top: 10px">Price range is: <span class="merchant-info">' . get_option( 'merchant_settings' )->MinAmount . ' to ' . get_option( 'merchant_settings' )->MaxAmount . '</span> </b>
				...
				if ( 'title' !== $this->get_field_type( $field ) ) {
					try {
						if ( 'general_setting_section' === $key ) {
							...
							$this->settings['splitit_refund_strategy']               = $this->get_field_value( 'splitit_refund_strategy', $field['splitit_refund_strategy'], $post_data );
			...
			try {
				...
				if (!empty($raw_post_data)) {
					...
					if (!empty($decoded_data)) {
                        $amount = $decoded_data['RefundSummary']['SucceedAmount'];
										...
										if ( 'refund' == $requested_action_type ) {
											...
											if ( ( empty( $refunds ) || $order->get_remaining_refund_amount() > 0 ) && in_array( $order->get_status(), array(
												...
												if ( $order->get_remaining_refund_amount() >= $amount ) {
													...
													$option_name = 'splitit_refund_data_' . $order_id;
													$refund_data_json = get_option($option_name);

													$line_items = array();

													if ($refund_data_json) {
														$refund_data = json_decode($refund_data_json, true);
														if ($refund_data) {
															foreach ($order->get_items() as $item_id => $item) {
																foreach ($refund_data as $refund_item) {
																	SplitIt_FlexFields_Payment_Plugin_Log::log_to_file( '$item_id: ' . $item_id);
																	SplitIt_FlexFields_Payment_Plugin_Log::log_to_file( '$refund_item: ' . json_encode($refund_item));
																	if ($refund_item['item_id'] == $item_id) {
																		$line_items[$item_id] = array(
																			'qty'          => $refund_item['qty'],
																			'refund_total' => $refund_item['refund_total'],
																			'refund_tax'   => $refund_item['refund_tax'],
																		);
																		break;
																	}
																}
													...
													SplitIt_FlexFields_Payment_Plugin_Log::log_to_file( 'line_items_for_order_id_' . $order_id . '_refund_id_' . $refund_id . ': ' . json_encode( $line_items ) );
											...
											$order->add_order_note( 'Cancel for the amount = ' . $amount . ' is succeeded on the Splitit side' );
											SplitIt_FlexFields_Payment_Plugin_Log::log_to_file( 'Cancel success. Amount = ' . $amount . ', Order ID: ' . $order_id . ' Refund ID: ' . $order_refund_id . ', ipn = ' . $ipn );
			...
			try {
				...
				if ( ! SplitIt_FlexFields_Payment_Plugin_Log::check_exist_order_by_ipn( $ipn ) ) {
                    ...
                    if ( $verify_data->getIsAuthorized() ) {
						...
						if ( $order->get_payment_method() == 'splitit' ) {
                            $order->update_status( $this->settings['splitit_auto_capture'] ? 'processing' : 'pending' );
						}
						...
						$order_id = $order_id_by_ipn->order_id ?? $order_by_transaction->order_id;
						$order                = wc_get_order( $order_id );
		...
		public function flex_field_initiate_method() {
			...
			if ( isset( $_POST ) ) {
				...
				$post_data['currency_code']        = !empty( $_POST['currency'] ) ? wc_clean( $_POST['currency'] ) : get_woocommerce_currency();
		...
		private function get_current_order_total() {
            ...
            $total = $order ? (float) $order->get_total() : $this->get_order_total();
		...
		public function checkout_validate() {
			if ( isset( $_POST ) ) {
				...
				if ( isset( $all_fields['g-recaptcha-response'] ) && $all_fields['g-recaptcha-response'] == '' ) {
					$errors[] = '<li>' . __( 'I`m not a robot is a required field.', 'woocommerce' ) . '</li>';
				}

                if ( isset( $all_fields['cf-turnstile-response'] ) && $all_fields['cf-turnstile-response'] == '' ) {
					$errors[] = '<li>' . __( 'Verify you are human is a required field.', 'woocommerce' ) . '</li>';
				}
					...
					switch ( $key ) {
						...
						case 'billing_state':
							...
							if ( isset( $wc_fields[ $key ]['required'] ) && $wc_fields[ $key ]['required'] ) {
								if ( ! empty( $valid_states ) && is_array( $valid_states ) && count( $valid_states )) {
		...
		public function add_order_data_to_db( $data ) {
			if ( isset( $data ) ) {
				global $woocommerce;
                $session = WC()->session;
				$fetch_session_item   = $session->get( 'chosen_shipping_methods' );
                $shipping_method_id = ! empty( $fetch_session_item ) ? explode( ':', $fetch_session_item[0] )[0] : '';
				$coupon_code           = '';
				$coupon_amount         = '';
                ...
                $cart = WC()->cart;
				$total_tax_amount  = '';
				$total_taxes_array = $cart->get_taxes();
                ...
                $insert_data = array(
                    ...
                    'user_id'                      => get_current_user_id(),
                    'cart_items'                   => json_encode( $cart->get_cart() ),
                    'shipping_method_cost'         => $cart->shipping_total,
                    ...
                    'set_shipping_total'           => $cart->shipping_total,
                    'set_discount_total'           => $cart->get_cart_discount_total(),
                    'set_discount_tax'             => $cart->get_cart_discount_tax_total(),
                    'set_cart_tax'                 => $cart->tax_total,
                    'set_shipping_tax'             => $cart->shipping_tax_total,
                    'set_total'                    => $cart->total,
                    'wc_cart'                      => json_encode( $cart ),
                    'get_packages'                 => json_encode( WC()->shipping->get_packages() ),
                    'chosen_shipping_methods_data' => json_encode( $session->get( 'chosen_shipping_methods' ) ),
		...
		public function init_ipn_to_the_thank_you_page() {
			add_filter('woocommerce_thankyou_order_received_text', array($this, 'splitit_add_installment_plan_number_data_thank_you_title',), 10, 2);
		}
	}
