...
function check_splitit_refund_status_by_ipn_daily() {
	...
	try {
		...
		foreach ( $orders_waiting_refund as $item ) {
			...
			SplitIt_FlexFields_Payment_Plugin_Log::log_to_file( json_encode($ipn_info) );

			$ipn_status = $ipn_info->getStatus();
			...
			$time_difference = time() - strtotime( $item->updated_at );
			...
			if ( $ipn_info->getRefOrderNumber() == $order_id ) {
						...
						if ( $refund_id == $splitit_refund_id ) {
							...
							if ( 'Succeeded' == $refund_status ) {
								...
								if ( $splitit_refund_amount == $refund_amount ) {
									...
									} elseif ( 'cancel' == $item->action_type ) {
									...
									if ( 'Cleared' == $ipn_status && 0 == $splitit_refund_amount && 'Succeeded' == $refund_status ) {
										if ( 'refund' == $item->action_type ) {
											$order->add_order_note( 'Refund has already been completed for this order on Splitit side' );
											$order->update_status( 'cancelled' );
										} elseif ( 'cancel' == $item->action_type ) {
											$order->add_order_note( 'Cancel has already been completed for this order on Splitit side' );
											$order->update_status( 'cancelled' );
										}
									} else {
