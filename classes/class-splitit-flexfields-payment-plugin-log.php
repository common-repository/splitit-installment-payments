...
class SplitIt_FlexFields_Payment_Plugin_Log {
	 ...
	 */
	public static function select_from_refund_log_by_ipn_and_refund_id( $ipn, $refund_id) {
		global $wpdb;
		$table_name = $wpdb->prefix . self::$db_table_refund_info_log;

		$sql = $wpdb->prepare(
			'SELECT id, order_id, refund_id, action_type FROM ' . $table_name . ' WHERE ipn = %s AND refund_id = %s  LIMIT 1',
			array(
				$ipn,
				$refund_id,
			)
	 ...
	 * @return array|object|void|null
	 */
	public static function select_from_refund_log_orders_without_refund_result() {
		global $wpdb;
		$table_name = $wpdb->prefix . self::$db_table_refund_info_log;
	 ...
	 * @return array|object|void|null
	 */
	public static function select_from_refund_log_by_order_id( $order_id) {
		global $wpdb;
		$table_name = $wpdb->prefix . self::$db_table_refund_info_log;

		$sql = $wpdb->prepare(
			'SELECT id FROM ' . $table_name . ' WHERE order_id = %s LIMIT 1',
			array(
				$order_id,
			)
		);
	...
	public static function get_splitit_info_by_order_id( $order_id ) {
		...
		return $splitit_transaction_info[0] ?? false;
	}
	...
	public static function add_order_data( $data ) {
		...
		$sql = $wpdb->prepare(
			'SELECT id FROM ' . $table_name . ' WHERE ipn = %s ORDER BY ipn DESC LIMIT 0,1',
			array(
				$data['ipn'],
			)
		);
	...
	public static function check_exist_order_by_ipn( $ipn ) {
		...
		return ! empty( $order_id[0]->order_id );
	}
