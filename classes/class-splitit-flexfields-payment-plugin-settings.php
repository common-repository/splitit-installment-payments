...
class SplitIt_FlexFields_Payment_Plugin_Settings {
	...
	public static function get_fields( $settings ) {
		return array(
			...
			'Payment_Method_Settings_section'         => array(
				...
				'splitit_refund_strategy'               => array(
					'title'   => __( 'Refund Strategy', 'splitit_ff_payment' ),
					'type'    => 'select',
					'class'   => 'wc-enhanced-select',
					'css'     => 'width: 420px; margin-bottom: 1px;',
					'default' => '',
					'desc_tip' => false,
					'options' => self::refund_strategy_selection(),
				),
	...
	public static function get_admin_scripts_and_styles( $plugin_id = 'splitit' ) {
		...
		$section = wc_clean( $_GET['section'] ?? null );
		$action  = wc_clean( $_GET['action'] ?? null );
		$post    = wc_clean( $_GET['post'] ?? null );
		$id      = wc_clean( $_GET['id'] ?? null );

		if ( $section == $plugin_id ) {
			add_action(
			...
			if ( ! get_option( 'splitit_logged_user_data' ) || ! get_option( 'api_key' ) ) {
				add_action( 'admin_footer', array( 'SplitIt_FlexFields_Payment_Plugin_Settings', 'welcome_pop_up' ) );
			}
		}

		if ( ( 'edit' === $action && isset( $post ) ) || ( 'edit' === $action && isset( $id ) ) ) {
			add_action(
				'admin_enqueue_scripts',
				array(
					'SplitIt_FlexFields_Payment_Plugin_Settings',
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="welcome-img-block">
						<div>
							<img src="<?php echo plugins_url( 'assets/img/welcome-connect.png', dirname( __FILE__ ) ); ?>" class="Group">
						</div>
						<div class="Path-2"></div>
						<div>
							<img src="<?php echo plugins_url( 'assets/img/welcome-config.png', dirname( __FILE__ ) ); ?>" class="Group">
						</div>
						<div class="Path-2"></div>
						<div>
							<img src="<?php echo plugins_url( 'assets/img/welcome-setup.png', dirname( __FILE__ ) ); ?>" class="Group">
						</div>
					</div>
	...
	public static function wpb_hook_javascript() {
		...
		<script
			api-key="<?php echo esc_attr( get_option( 'api_key' ) ? get_option( 'api_key' ) : '' ); ?>"
	...
	/**
	 * Return allowed sections for Upstream Messaging
	 *
	 * @return array
	 */
	...
	/**
	 * Return allowed sections for Refund Strategy
	 *
	 * @return array
	 */
	private static function refund_strategy_selection() {
		return array_reduce(
			\Splitit\Model\RefundStrategy::getAllowableEnumValues(),
			function( $carry, $item ) {
				$value          = preg_replace( '/([a-z])([A-Z])/', '$1 $2', $item );
				$carry[ $item ] = ucfirst( strtolower( $value ) );
				return $carry;
			},
			array()
		);
	...
	public static function update_order_status_to_old( $order ) {
		$order->update_status( $order->get_meta( '_old_status' ) );
	}
}
