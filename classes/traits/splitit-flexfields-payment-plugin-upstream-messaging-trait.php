...
trait SplitIt_FlexFields_Payment_Plugin_UpstreamMessagingTrait {
	...
	public function um_on_shop_page() {
		if ( $this->is_enabled() && $this->is_upstream_messaging_selection( 'shop' ) && ( is_shop() || is_product_category() || is_tax( 'product_brand' ) ) ) {
			...
			$um_block = is_array( $this->splitit_upstream_messaging_position_shop_page ) ? $this->generate_um_block( 'shop_page', $this->splitit_upstream_messaging_position_shop_page, $price, $installments, $hide_upstream_message, '_' .  $product->get_id() . '' ) : '';
	...
	private static function make_block( $tag_name, $page_config, $hide_upstream_message = '' ) {
		...
		foreach ( $page_config as $key => $config ) {
            if ( 'enable_one_liner' == $key || 'regular' == $key || '' == $config) continue;
	...
	public function display_um_with_custom_selector ( $type, $selector, $um ) {
        ...
        document.addEventListener("DOMContentLoaded", function() {
            if (! document.querySelector('<?php echo $type; ?>')) {
                let custom_selector = document.querySelector('<?php echo $selector; ?>');

                if ( custom_selector) {
                    localStorage.setItem( 'um_type', '<?php echo $type; ?>' )
                    localStorage.setItem( 'custom_selector', '<?php echo $selector; ?>' )
                    localStorage.setItem( 'um', <?php echo $um; ?> )
                    let umElement = document.createElement('div');
                    umElement.innerHTML = <?php echo $um; ?>;
                    custom_selector.appendChild(umElement);
                }
            }
        })
        </script>
	...
	public function cart_page( $custom_selector = false ) {
		if ($this->is_upstream_messaging_selection( 'cart' )) {
			$message_block = '';
			$price         = custom_wc_price_value( $this->get_cart_total() );
			$installments  = $this->get_installment_by_price( $price );
        ...
        <script>
            ...
            jQuery(document.body).on('updated_cart_totals', function () {
                jQuery.ajax({
                    ...
                    success: function (response) {
                        let um_banner = jQuery( localStorage.getItem( 'um_type' ) )
                        if ( !um_banner.length ) {
                            let element         = jQuery( localStorage.getItem( 'custom_selector' ) )
                            if (element.length) {
                                element.append(localStorage.getItem( 'um' ))
                            }
                        }
	...
	public function calculate_new_installment_price_product_page() {
		...
		if ( $this->is_enabled() && $this->is_upstream_messaging_selection( 'product' ) ) {
			...
			if ( ! empty( $price ) && 'choose an option' !== $price ) {
				$installments = $this->get_installment_by_price( $price );
			} else {
				$price        = 0;
				$installments = 0;
	...
	public function get_array_of_installments( $price ) {
		if ( $this->is_enabled() && ! empty( $this->splitit_inst_conf ) ) {
			...
			if ( isset( $this->splitit_inst_conf['ic_installment'] ) ) {
				if ( array_key_exists( $key, $this->splitit_inst_conf['ic_installment'] ) ) {
					return array_unique( explode( ',', $this->splitit_inst_conf['ic_installment'][ $key ] ) );
				}
			}
		}
	...
	public function check_if_price_in_range() {
		...
		if ( isset( $this->splitit_inst_conf['ic_installment'] ) && array_key_exists( $key, $this->splitit_inst_conf['ic_installment'] ) ) {
			return ! empty( $this->splitit_inst_conf['ic_installment'][ $key ] );
		}
	...
	public function get_splitit_per_product_settings() {
        $option = (int) ( $this->settings['splitit_product_option'] ?? 0 );
        $product_ids = $this->settings['splitit_products_list'] ?? [];
		$disabled    = false;
	...
	public function is_allowed_payment_per_products() {
		...
		foreach ( $cart_items as $cart_item ) {
			...
			if ( ( ! empty( $sku ) && in_array( $sku, $this->get_product_skus_from_ids( $product_ids ) ) )
