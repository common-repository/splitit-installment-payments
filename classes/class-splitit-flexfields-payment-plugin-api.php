...
class SplitIt_FlexFields_Payment_Plugin_API {
	...
	public function __construct( $settings, $default_number_of_installments = null ) {
		...
		$this->refund_strategy                = $settings['splitit_refund_strategy'] ?? \Splitit\Model\RefundStrategy::FUTURE_INSTALLMENTS_FIRST;
	...
	public function login( $check_credentials = false ) {
		...
		try {
			...
			curl_setopt_array(
				$curl,
				array(
					CURLOPT_URL            => 'https://id.' . $environment . '.splitit.com/connect/token',
					CURLOPT_HTTPHEADER     => array( 'Content-Type: application/x-www-form-urlencoded' ),
					CURLOPT_SSL_VERIFYPEER => false,
					CURLOPT_RETURNTRANSFER => true,
					CURLOPT_POST           => true,
					CURLOPT_POSTFIELDS     => "client_id=$client_id&client_secret=$client_secret&grant_type=client_credentials&scope=api.v3",
				)
			);
	...
	public function initiate( $data ) {
		...
		try {
			...
			if ( isset( $access_token ) && ! isset( $access_token['error'] ) ) {
				...
				if (! empty( $data['numberOfInstallments'] ) ) {
				...
				if ( $success ) {
					...
					SplitIt_FlexFields_Payment_Plugin_Log::save_log_info( $data, 'Successful initiate', 'info' );
	...
	public function update( $order_id, $ipn ) {
		...
		while ( $attempt <= $max_attempts ) {
			try {
				...
				SplitIt_FlexFields_Payment_Plugin_Log::save_log_info( $data, __( 'Update was successful', 'splitit_ff_payment' ), 'info' );
	...
	public function refund( $amount = null, $currency_code = '', $ipn = '', $order_id = '', $reason = '', $action_type = '' ) {
		...
		SplitIt_FlexFields_Payment_Plugin_Log::log_to_file( 'RefundStrategy: ' . $this->refund_strategy );
		...
		if ( $response->getRefundId() && $response->getSummary()->getFailedAmount() == 0 ) {
			...
			SplitIt_FlexFields_Payment_Plugin_Log::save_log_info( $data, 'Refund was successful, no failed amount', 'splitit_ff_payment' );
	...
	public function cancel( $installment_plan_number ) {
		...
		if ( $response->getInstallmentPlanNumber() ) {
			...
			SplitIt_FlexFields_Payment_Plugin_Log::save_log_info( $data, 'Canceled was successful', 'splitit_ff_payment' );
	...
	public function start_installments( $installment_plan_number, $order_id ) {
		...
		try {
			...
			$result = $api_instance->installmentPlan->updateOrder(
				...
				'',
				$order_id,
                \Splitit\Model\ShippingStatus::SHIPPED,
				true
			);
			...
			if ( $result->getStatus() == \Splitit\Model\PlanStatus::ACTIVE ) {
				...
				SplitIt_FlexFields_Payment_Plugin_Log::save_log_info( $data, 'StartInstallment was successful', 'splitit_ff_payment' );
	...
	public function verify_payment( $installment_plan_number ) {
		...
		while ( $attempt <= $max_attempts ) {
			...
			try {
				...
				SplitIt_FlexFields_Payment_Plugin_Log::save_log_info( $data, __( 'VerifyPayment was successful', 'splitit_ff_payment' ), 'info' );
